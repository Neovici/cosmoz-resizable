import { component, useEffect, useMemo, useRef } from '@pionjs/pion'
...
import { usePersist, localStorageAdapter } from './hooks/use-persist'
import {
	ResizerDirection,
	SizeSpec,
	PersistAdapter,
...
const ResizableView = ({
	...
	useImperativeApi(
		{
			expandTo: (minSizePx: number) => {
				const panels = panelsRef.current
				if (!panels) return
				const containerSize = containerSizeOf(host, direction)
				const nextPx =
					containerSize - (ratioRef.current ?? 0.5) * containerSize
				if (nextPx >= minSizePx) return
				const bounds = resolveBounds(minSize, maxSize, containerSize)
				const splitPx = clampSplitPx(containerSize - minSizePx, bounds)
				const { ratios } = applySizes(
					panels.previous,
					panels.next,
					splitPx,
		...
		[direction, minSize, maxSize, persistRatio]
	)
