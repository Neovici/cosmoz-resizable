...
### `useResizable` hook
Imperative preset replacing `useSplit` from cosmoz-queue. Wires a split between `#list` and `#queue` in the host's shadow DOM when `activeTab === 'split'`.

```js
useResizable({ activeTab, initialSizes: [0.3, 0.7], minSize: 200 })
```
