...
describe('cosmoz-resizable-view', () => {
	...
	describe('slot reassignment', () => {
		// Drags the resize handle to x (absolute page X)
		const dragHandle = (el, x) => {
			const handle = el.shadowRoot.querySelector('cosmoz-resize-handle');
			...
			handle.dispatchEvent(
				new CustomEvent('resize-handle', {
					bubbles: true,
					detail: { phase: 'move', mousePosition: { x, y: 100 } },
				}),
			);
			handle.dispatchEvent(
				new CustomEvent('resize-handle', {
					bubbles: true,
					detail: { phase: 'end', mousePosition: { x, y: 100 } },
				}),
			);
		};

		it('tracks the cursor after slot reassignment', async () => {
			const el = await fixture(
				html`<cosmoz-resizable-view
					style="display:flex; width:800px; height:300px;"
				>
					<div id="a" slot="previous" style="min-width:50px">A</div>
					<div id="b" slot="next" style="min-width:50px">B</div>
				</cosmoz-resizable-view>`,
			);
			...
			// drag in the initial assignment: #a is previous
			dragHandle(el, containerLeft() + 400);
			await waitUntil(
				() =>
					Math.round(el.querySelector('#a').getBoundingClientRect().width) ===
					400,
				'initial drag should set #a basis to 400px',
				{ timeout: 1500 },
			);

			// swap the slots: #b becomes previous
			el.querySelector('#a').setAttribute('slot', 'next');
			el.querySelector('#b').setAttribute('slot', 'previous');
			await waitUntil(
				() =>
					el.shadowRoot
						.querySelector('slot[name="previous"]')
						.assignedElements()[0]?.id === 'b',
				undefined,
				{ timeout: 1500 },
			);

			// wait for the rebind effect: stale basis on #a cleared
			await waitUntil(
				() => el.querySelector('#a').style.flexBasis === '',
				undefined,
				{ timeout: 1500 },
			);

			// drag again: #b (the new previous) must track the cursor exactly
			dragHandle(el, containerLeft() + 300);
			await waitUntil(
				() =>
					Math.round(el.querySelector('#b').getBoundingClientRect().width) ===
					300,
				'#b should track the drag after reassignment',
				{ timeout: 1500 },
			);
			expect(
				Math.round(el.querySelector('#b').getBoundingClientRect().width),
		...
		it('clears stale flex-basis from the other slot on reassignment', async () => {
			const el = await fixture(
				html`<cosmoz-resizable-view
					style="display:flex; width:800px; height:300px;"
				>
			...
			dragHandle(el, containerLeft() + 400);
			await waitUntil(
				() => el.querySelector('#a').style.flexBasis === '400px',
				undefined,
				{ timeout: 1500 },
			);

			// swap: #a moves to next, #b becomes previous
			el.querySelector('#a').setAttribute('slot', 'next');
			el.querySelector('#b').setAttribute('slot', 'previous');
			await waitUntil(
				() =>
			...
			// #b never had a drag basis; #a's stale 400px basis (now in next)
			// must have been cleared — otherwise the container overflows and
			// flex-shrink eats the next drag
			await waitUntil(
				() => el.querySelector('#a').style.flexBasis === '',
				'stale basis on #a should be cleared',
				{ timeout: 1500 },
			);
