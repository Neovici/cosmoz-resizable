...
describe('cosmoz-resizable-view', () => {
	it('is defined', async () => {
		const el = await fixture(html`<cosmoz-resizable-view></cosmoz-resizable-view>`)
		expect(el.tagName.toLowerCase()).to.equal('cosmoz-resizable-view')
	})

	it('inserts a handle between 2 slotted children', async () => {
		const el = await fixture(html`<cosmoz-resizable-view>
	...
	it('applies initial flex-basis to panels', async () => {
		const el = await fixture(html`<cosmoz-resizable-view .initialSizes=${[0.3, 0.7]}>
			<div id="prev" style="background:red">prev</div>
			<div id="next" style="background:blue">next</div>
		</cosmoz-resizable-view>`)
		await waitUntil(() => el.querySelector('cosmoz-resize-handle'), undefined, {
	...
	it('expandTo expands next panel if smaller than minSize', async () => {
		const el = await fixture(html`<cosmoz-resizable-view
			style="display:flex; width:1000px; height:600px;"
			.initialSizes=${[0.9, 0.1]}
		>
		...
		el.expandTo(300)
		const next = el.querySelector('#next')
		const nextFlex = parseFloat(next.style.flexBasis)
		expect(nextFlex).to.be.at.least(29)
	})

	it('expandTo does nothing if next already >= minSize', async () => {
		const el = await fixture(html`<cosmoz-resizable-view
			style="display:flex; width:1000px; height:600px;"
		...
		const prev = el.querySelector('#prev')
		const before = prev.style.flexBasis
		el.expandTo(300)
		expect(prev.style.flexBasis).to.equal(before)
	})
